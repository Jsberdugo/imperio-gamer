...
import { CheckCircle2, Home, ShoppingBag, UserPlus, Package } from 'lucide-react';
import type { PeripheralPaymentData } from './PeripheralCheckoutPage';
...
interface PeripheralReceiptPageProps {
  cartItems: PeripheralCartItem[];
  paymentData: PeripheralPaymentData;
  isLoggedIn: boolean;
  onNavigate: (page: string) => void;
}

export function PeripheralReceiptPage({ cartItems, paymentData, isLoggedIn, onNavigate }: PeripheralReceiptPageProps) {
  const receiptNumber = `RCP-P-${Date.now().toString().slice(-8)}`;
        ...
        >
              ...
              {cartItems.map((item) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm">{item.peripheral.name}</p>
                      <p className="text-xs text-muted-foreground">
                        {item.peripheral.brand} - Cantidad: {item.quantity}
                      </p>
                    </div>
                  </div>
                  <p className="text-primary">${(item.peripheral.price * item.quantity).toFixed(2)}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="mb-6">
            ...
            >
              <div className="flex justify-between">
                ...
                <span>{paymentData.deliveryInfo.fullName}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">Cédula:</span>
                <span>{paymentData.deliveryInfo.idNumber}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">Teléfono de contacto:</span>
                <span>{paymentData.deliveryInfo.contactNumber}</span>
              </div>
              <div className="pt-2 border-t border-primary/20">
                <p className="text-muted-foreground text-sm mb-1">Dirección de entrega:</p>
                <p className="text-sm">{paymentData.deliveryInfo.deliveryAddress}</p>
              </div>
            </div>
          </div>
          ...
          <div className="mt-6 p-4 border border-primary/20 bg-secondary/30">
            <p className="text-sm text-muted-foreground text-center">
              Tu pedido será enviado en 3-5 días hábiles. Nos pondremos en contacto contigo para coordinar la entrega.
            </p>
          </div>
        </div>
